...
st.caption("Paste your Sheet URL/ID or use ?sheet_id=... in the URL. This build auto-fills missing token_uri/auth_uri in your service account secrets.")
...
def normalize_private_key(sa: dict) -> dict:
    ...
    # Required defaults if omitted in Secrets UI
    sa.setdefault("type", "service_account")
...
def read_service_account():
    if "gcp_service_account" not in st.secrets:
        ...
        st.stop()
    sa = normalize_private_key(st.secrets["gcp_service_account"])
    # Friendly checks
    missing = [k for k in ("client_email","private_key") if not sa.get(k)]
    if missing:
        st.error(f"Your secrets are missing: {', '.join(missing)}")
        st.stop()
    return sa
...
with st.sidebar:
    ...
    st.write("Service account has token_uri:", bool(SERVICE_ACCOUNT_INFO.get("token_uri")))
    st.write("Service account has auth_uri:", bool(SERVICE_ACCOUNT_INFO.get("auth_uri")))
